...
# # Define Pydantic model for query request
...
#     file_path = os.path.join(UPLOAD_FOLDER, f"{month}-{year}.csv")
...
#     category_spending = df.groupby("Category")["Withdrawal Amt."].sum().reset_index()
...
#         logging.error(f"Error with Gemini API: {response.status_code} - {response.text} {API_KEY}")
...
#     if not file_path:
#         logging.error(f"No data found for {request.month} {request.year}.")
